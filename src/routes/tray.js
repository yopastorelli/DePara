...
router.post('/minimize', async (req, res) => {
    try {
        ...
        const command = `wmctrl -l | grep -E "(DePara|localhost:3000|Chromium|Chrome|Firefox)" | awk '{print $1}' | while read window_id; do wmctrl -i -r "$window_id" -b add,hidden 2>/dev/null; done`;
